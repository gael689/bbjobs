...
- **Row-Level Security**: `app/db/rls.py` sets `SET LOCAL app.current_user_id` / `app.user_role` per request so Postgres RLS policies scope data access (e.g. a candidate's profile is only visible to companies they applied to).
